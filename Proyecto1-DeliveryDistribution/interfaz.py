...
def calcular_tiempo_rutas(rutas):
    try:
        ...
        velocidad = int(entry_velocidad_media.get())  # Velocidad en km/h
        if velocidad <= 0:
    ...
    for i, (ruta, distancia_total) in enumerate(rutas_unicas, start=1):
        ...
        if tiempo_total_horas <= horas_conduccion_diaria:
            ...
            if horas_restantes > 0:
                tiempo_descanso += descanso_diario
            tiempo_total = tiempo_total_horas + tiempo_descanso
            dias = int(tiempo_total // 24)
            horas = round(tiempo_total % 24)
            if horas == 24:
                dias += 1
                horas = 0
